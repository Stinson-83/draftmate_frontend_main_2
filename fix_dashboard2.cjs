...
const startIndex = lines.findIndex(l => l.includes('const handleImageChange = (e) => {'));
const endIndex = lines.findIndex((l, i) => i > startIndex && l.includes('toast.error(err.message || \'Failed to save profile\');'));

if (startIndex !== -1 && endIndex !== -1) {
    const replacementLines = `    const handleImageChange = (e) => {
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            ...
            toast.error(err.message || 'Failed to save profile');`.split('\n');
